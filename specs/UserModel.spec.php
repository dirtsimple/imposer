...
describe("UserModel", function(){
	...
	describe("lookup_by_email()", function() {
		it("gets an ID from get_user_by('email', key)", function(){
			fun\expect('get_user_by')->once()->with('email', 'me@u')->andReturn((object) array('ID'=>21));
			fun\expect('get_user_by')->once()->with('email', 'u@me')->andReturn(false);
	...
	describe("lookup_by_login()", function() {
		it("gets an ID from get_user_by('login', key)", function(){
			fun\expect('get_user_by')->once()->with('login', 'me')->andReturn((object) array('ID'=>17));
			fun\expect('get_user_by')->once()->with('login', 'you')->andReturn(false);
	...
	describe("save()", function() {
		...
		it("calls wp_update_user if id is known", function(){
			...
			fun\expect('wp_update_user')->once()->with( $this->expected )->andReturn(16);
			expect( Promise::interpret($this->model->apply()) )->to->equal(16);
		});
		it("calls wp_insert_user if new", function(){
			fun\expect('wp_insert_user')->once()->with( $this->expected )->andReturn(99);
